...
import com.clearner.youtube.HttpRequest;
...
public class EtagCache implements Flushable {
  ...
  private static String getKey(final String uri) {
    final byte[] input;
    try {
      input = uri.getBytes(HttpRequest.CHARSET_UTF8);
  ...
  private static boolean isCacheable(final HttpURLConnection connection) {
    try {
      return HttpRequest.METHOD_GET.equals(connection.getRequestMethod())
  ...
  public InputStream put(final URLConnection connection,
    ...
    String etag = connection.getHeaderField(HttpRequest.HEADER_ETAG);
    if (etag == null || etag.length() == 0)
      return null;
    ...
    try {
      ...
      if (!(input instanceof InflaterInputStream)
          && HttpRequest.ENCODING_GZIP.equals(connection
              .getHeaderField(HttpRequest.HEADER_CONTENT_ENCODING)))
